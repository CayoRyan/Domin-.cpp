...
//atrav�s da selection sort ordena a mao
...
//Verifica se a pedra escolhida � encaix�vel e executa a a��o caso for
...
int cava(tp_pilha *monte, tp_listad *mao){
	...
	if(pop(monte, &aux)){
		printf("Essa e a pe�a que vc cavou: %d/%d\n", aux.lado1, aux.lado2);
...
//Checa se as condicoes pra finalizar o jogo foram alcan�adas
...
int main() {
	...
	tp_listad *mao1, *mao2, *mesa;//Cria as listas para as m�os dos jogadores
	...
	cria_pedras(pedras); //Chama a fun��o que preenche o vetor com os dois lados das pedras
	embaralha_pedras(pedras); //Chama a fun��o que embaralha o vetor com as pedras
  
	//Preenche o monte com todas as pedras j� embaralhadas
	for(n=0;n<=27;n++){
		push(&monte,pedras[n]);
	}
